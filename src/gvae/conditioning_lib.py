...
def add_months(condition_kwargs, condition_set, raw_dates=None):
    if raw_dates is None:
        raise ValueError("Raw dates must be provided.")
    ...
    print(
        f"condition_set keys after adding months: {list(condition_set.keys())}"
    )  # ADD THIS
...
def add_weekdays(condition_kwargs, condition_set, raw_dates=None):
    ...
    print(
        f"condition_set keys after adding weekdays: {list(condition_set.keys())}"
    )  # ADD THIS
...
def add_temperature(
    condition_kwargs, condition_set, dataset_path=None, raw_dates=None
):
    ...
    condition_kwargs["supports"].append(
        [
            df_temp[condition_kwargs["tags"][-1]].min(),
            df_temp[condition_kwargs["tags"][-1]].max(),
        ]
    )
    ...
    condition_set["temp_max_delta"] = df_temp[
        condition_kwargs["tags"][-1]
    ].values[..., None]
...
def add_users(
    ...
    data=None,
    dataset_path=None,
    user_embedding_kwargs=None,
    config_dict=None,
):
    if dataset_path is None:
        raise ValueError("Dataset path must be provided.")
    if user_embedding_kwargs is None:
        raise ValueError("User embedding kwargs must be provided.")
    if data is None:
    ...
    model_kwargs, fit_kwargs = (
        ...
        user_embedding_kwargs["fit_kwargs"],
    )
    ...
    if model_dir is not None:
        ...
        model_dir = os.path.join(
            base_dir, f"model_{len(os.listdir(base_dir)) + 1}"
        )  # this line requires the exitences of a folder in the dataset.csv dir named user_encoding_models
    ...
    condition_kwargs["supports"].append(
        [fit_kwargs["lda"]["doc_topic_prior"], user_model.doc_lengths.max()]
    )
    condition_set["users"] = user_gamma.repeat(num_days, axis=0)
    print(
        f"condition_set keys after adding users: {list(condition_set.keys())}"
    )  # ADD THIS
...
def add_absmean(condition_kwargs, condition_set, data=None):
    ...
    condition_kwargs["types"].append("cont")
    condition_kwargs["supports"].append(
        [np.nanmin(absmean), np.nanmax(absmean)]
    )
...
def add_day_befores(condition_kwargs, condition_set, data=None):
    if data is None:
        raise ValueError("Data must be provided.")
    day_befores = np.concatenate(
        [data[:, [-1], :] * np.nan, data[:, :-1, :]], axis=1
    ).reshape(
        -1, data.shape[-1]
    )  # circular shift
    ...
    condition_kwargs["supports"].append(
        [np.nanmin(day_befores), np.nanmax(day_befores)]
    )
...
def add_twoday_befores(condition_kwargs, condition_set, data=None):
    if data is None:
        raise ValueError("Data must be provided.")
    twoday_befores = np.concatenate(
        [data[:, -2:, :] * np.nan, data[:, :-2, :]], axis=1
    ).reshape(
        -1, data.shape[-1]
    )  # circular shift
    ...
    condition_kwargs["types"].append("identity")
    condition_kwargs["supports"].append(
        [np.nanmin(twoday_befores), np.nanmax(twoday_befores)]
    )
...
def add_week_befores(condition_kwargs, condition_set, data=None):
    if data is None:
        raise ValueError("Data must be provided.")
    week_befores = np.concatenate(
        [data[:, -7:, :] * np.nan, data[:, :-7, :]], axis=1
    ).reshape(
        -1, data.shape[-1]
    )  # circular shift
    ...
    condition_kwargs["types"].append("identity")
    condition_kwargs["supports"].append(
        [np.nanmin(week_befores), np.nanmax(week_befores)]
    )
...
def add_month_befores(condition_kwargs, condition_set, data=None):
    if data is None:
        raise ValueError("Data must be provided.")
    month_befores = np.concatenate(
        [data[:, -7 * 4 :, :], data[:, : -7 * 4, :]], axis=1
    ).reshape(
        -1, data.shape[-1]
    )  # circular shift
    ...
    condition_kwargs["types"].append("identity")
    condition_kwargs["supports"].append(
        [np.nanmin(month_befores), np.nanmax(month_befores)]
    )
...
def add_pulse_cluster(condition_kwargs, condition_set, dataset_path=None):
    ...
    print(dataset_path)
    if "PULSE_WIDE" in str(dataset_path):
        # PULSE_WIDE datasets aren't laid out as a literal
        # <dataset_path>/dataset.parquet -- read the same way
        # get_full_data()'s PULSE_WIDE branch does, via the shared loader,
        # so row order/filtering matches exactly and cluster labels stay
        # aligned with the daily profiles built there.
        dataset_dir, tier, variant = pulse_wide.parse_pulse_wide_path(dataset_path)
        df, _ = pulse_wide.load_pulse_wide_frame(dataset_dir, tier, variant)
        cluster = df["cluster"].values
    else:
        cluster = read_dataset(dataset_path, usecols=["cluster"])["cluster"].values
    # Numeric-encode (like add_is_weekend's 0/1) -- downstream code (e.g.
    # prepare_data()'s missing-condition union) calls np.isnan() over every
    # condition_set array uniformly, which raises on a raw string dtype.
    cluster_vocab = sorted(np.unique(cluster).tolist())
    cluster_to_id = {c: i for i, c in enumerate(cluster_vocab)}
    cluster_ids = np.array([cluster_to_id[c] for c in cluster])
    condition_kwargs["tags"].append("pulse_cluster")
    condition_kwargs["types"].append("cat")
    condition_kwargs["supports"].append(list(range(len(cluster_vocab))))
    condition_set["pulse_cluster"] = cluster_ids[..., None]


def _period_sum(raw_data, raw_dates, freq):
    num_users, num_days = raw_data.shape[:2]
    daily = np.sum(raw_data, axis=-1)
    if freq == "D":
        return daily
    dates_2d = np.asarray(raw_dates).reshape(num_users, num_days)
    aggregate = np.empty_like(daily)
    for user in range(num_users):
        index = pd.DatetimeIndex(dates_2d[user])
        series = pd.Series(daily[user], index=index).sort_index()
        grouped = series.groupby(pd.Grouper(freq=freq)).transform("sum")
        aggregate[user] = grouped.reindex(index).to_numpy()
    return aggregate


def add_load_aggregate(
    condition_kwargs, condition_set, tag, raw_data=None, raw_dates=None, freq="D"
):
    if raw_data is None:
        raise ValueError("Raw data must be provided.")
    if freq != "D" and raw_dates is None:
        raise ValueError("Raw dates must be provided.")
    aggregate = _period_sum(raw_data, raw_dates, freq).reshape(-1)
    condition_kwargs["tags"].append(tag)
    condition_kwargs["types"].append("cont")
    condition_kwargs["supports"].append([np.nanmin(aggregate), np.nanmax(aggregate)])
    condition_set[tag] = aggregate[..., None]


def _select_dual_signal(raw_data, tag, config_dict):
    if raw_data is None:
        return None
    if config_dict is None or config_dict.get("read_mode") != "dual-read":
        return raw_data
    half = raw_data.shape[-1] // 2
    if tag.endswith("_odn") or tag == "gen_or_not":
        return raw_data[..., half:]
    return raw_data[..., :half]


def add_gen_or_not(condition_kwargs, condition_set, raw_data=None):
    if raw_data is None:
        raise ValueError("Raw data must be provided.")
    num_days = raw_data.shape[1]
    user_generates = np.sum(raw_data, axis=(1, 2)) > 0
    gen_or_not = np.repeat(user_generates.astype(int), num_days)
...
def prepare_conditions(
    ...
    condition_kwargs = {}
    (
        condition_kwargs["tags"],
        condition_kwargs["types"],
        condition_kwargs["supports"],
        condition_set,
    ) = ([], [], [], {})
    ...
    for condition_tag in condition_tag_list:
        if condition_tag == "months":
            ...
            add_temperature(
                condition_kwargs, condition_set, dataset_path, raw_dates
            )
        ...
        elif condition_tag in ("daily_ldn", "daily_odn"):
            add_load_aggregate(
                condition_kwargs, condition_set, condition_tag,
                raw_data=_select_dual_signal(raw_data, condition_tag, config_dict), freq="D",
            )
        elif condition_tag in ("weekly_ldn", "weekly_odn"):
            add_load_aggregate(
                condition_kwargs, condition_set, condition_tag,
                raw_data=_select_dual_signal(raw_data, condition_tag, config_dict),
                raw_dates=raw_dates, freq="W",
            )
        elif condition_tag in ("monthly_ldn", "monthly_odn"):
            add_load_aggregate(
                condition_kwargs, condition_set, condition_tag,
                raw_data=_select_dual_signal(raw_data, condition_tag, config_dict),
                raw_dates=raw_dates, freq="M",
            )
        elif condition_tag == "gen_or_not":
            add_gen_or_not(
                condition_kwargs, condition_set,
                _select_dual_signal(raw_data, condition_tag, config_dict),
            )
        else:
...
class Conditioner:
    ...
    def add_transformer(self, tag, support, typ, data=None):
        if typ == "circ":
            self.transformers[tag] = CircularTransformer(
                max_conds=np.max(support), min_conds=np.min(support)
            )
            self.cond_dim += 2
        elif typ == "cat":
            self.transformers[tag] = OneHotEncoder(sparse_output=False).fit(
                data
            )
            ...
            self.transformers[tag] = OrdinalEncoder(categories=[support]).fit(
                data
            )
            self.cond_dim += 1
        elif typ == "dir":
        ...
        print(
            f"for {tag} adding transformer of type {typ} with support {support}"
        )
    ...
    def transform(self, data, num_samples=1):
        ...
        if (
            "dir" in self.types
            and self.transformers[
                self.tags[self.types.index("dir")]
            ].transform_style
            == "sample"
        ):
            for tag in self.tags:
                data_ = data[tag]
                if tag == "users":
                    transformed_data_ = self.transformers[tag].transform(
                        data_, num_samples=num_samples
                    )
                else:
                    transformed_data_ = np.repeat(
                        self.transformers[tag].transform(data_)[None, ...],
                        num_samples,
                        axis=0,
                    )
            ...
            for tag in self.tags:
                transformed_data.append(
                    self.transformers[tag].transform(data[tag])
                )
            return np.concatenate(transformed_data, axis=-1)
    ...
    def get_random_conditions(self, num_samples=1, random_seed=None):
        ...
        for tag, typ, support in zip(self.tags, self.types, self.supports):
            if typ == "circ":
                random_conditions[tag] = np.random.randint(
                    self.transform423ers[tag].min_conds,
                    self.transformers[tag].max_conds + 1,
                    num_samples,
                )[..., None]
            elif typ == "cat" or typ == "ord":
                random_conditions[tag] = np.random.choice(
                    self.transformers[tag].categories_[0], num_samples
                )[..., None]
            elif typ == "cont":
                random_conditions[tag] = (
                    self.transformers[tag]
                    .inverse_transform(
                        np.random.rand(num_samples)[:, np.newaxis]
                    )
                    .squeeze(-1)[..., None]
                )
            elif typ == "dir":
                rnd_doc_length = np.random.uniform(
                    low=support[0], high=support[1], size=num_samples
                )
                random_conditions[tag] = (
                    np.random.dirichlet(
                        alpha=[1.0] * self.transformers[tag].num_dims,
                        size=num_samples,
                    )
                    * rnd_doc_length[:, np.newaxis]
                )
            else:
                raise ValueError("Unknown type.")
    ...
    def save(self, save_path):
        with open(os.path.join(save_path, "conditioner.pkl"), "wb") as f:
            pickle.dump(self, f)
        print(f"Conditioner saved to {save_path}")

    @staticmethod
    def load(folder_path):
        with open(os.path.join(folder_path, "conditioner.pkl"), "rb") as f:
